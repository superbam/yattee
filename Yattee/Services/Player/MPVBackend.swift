...
extension MPVBackend: MPVClientDelegate {
    ...
    private func reactivateAudioSession() {
        do {
            ...
            #if os(tvOS)
            // Keep the long-form audio route sharing policy on re-activation so
            // multi-speaker (AirPlay 2) routing isn't dropped back to the
            // default output. Must match setupAudioSession() in PlayerService.
